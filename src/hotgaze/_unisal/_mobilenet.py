...
# for HotGaze deep saliency inference. Unmodified from original.
...
class MobileNetV2(nn.Module):
    def __init__(self, widen_factor=1.0, pretrained=True, last_channel=None, input_channel=32):
        super().__init__()
        ...
        if self.last_channel is not None:
            output_channel = (
                int(self.last_channel * widen_factor) if widen_factor > 1.0 else self.last_channel
            )
